...
two properties Heuristic 1 asserts, SEPARATELY:
  ...
  (H1) support: the concatenated basis D_F has joint entry-support dimension <= lambda
        (the operational property; this is NOT equivalent to the stronger
        'every element is alpha V-valued');
  image rank (Theorem 2): rk_{F_qm}(G_pub D_F) = k  (proved, not heuristic).
...
def one_config(m, n1, k1, n2, k2, lam, N=100, layout="spread", base_seed=20000):
    ...
    for s in range(base_seed, base_seed + N):
        ...
        if sup is not None:
            sampled += 1
            supp_ok += (sup <= lam)          # (H1)
            rank_ok += (ir == k)             # image rank (Theorem 2)
...
def show(res):
    ...
    print(f"  m={res['m']:2} n1={res['n1']} lam={res['lam']} "
          ...
          f"(H1) support<=lam {res['supp_ok']}/{s} CI[{lo_s:.2f},{hi_s:.2f}]  "
          f"rank=k (Thm 2) {res['rank_ok']}/{s} CI[{lo_r:.2f},{hi_r:.2f}]  "
...
def campaign():
    ...
    print("   (H1) support test (the genuine heuristic)   image rank = k (Theorem 2)")
    ...
    configs = [
        ...
        # success_probability.py bounds only the guessing probability P_r, NOT the support
        # property; the large-gap regime is therefore a heuristic extrapolation, not covered
        # analytically. The n1=2
    ...
    print("""
Reading: (H1) is the only genuinely heuristic ingredient of the accelerated regime; it
holds throughout here, at the modest gaps these configurations reach (larger gaps need the
LARGE_M campaign of gabkron_attack.py). The image-rank part (Theorem 2) is
...
if __name__ == "__main__":
    campaign()nuinely wrong guess F (screened to contain no scalar multiple
of V) yields NO functional key. Conditioning on a good F lets us reach LARGE gaps
r - lambda that random guessing could never sample, and to vary lambda, n1 and the layout.

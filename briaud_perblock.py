...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--summary":
        ...
        # Toy sizes use a spread layout (both Kronecker blocks non-empty); build_instance
        # requires it. A concentrated layout such as (2,0) is not a spread instance and is
        # intentionally not used here.
        # (1,1) at n2=8,k2=4: r too large vs n_bl, so Eq.(4) is INFEASIBLE (gamma_max<lambda).
        run(seed=1000, m=8, n2=8, k2=4, lblocks=(1, 1))
        # (1,1) at n2=10,k2=4: Eq.(4) HOLDS (gamma_max=lambda=2); the per-block Briaud system
        # is solvable. At this small size the over-determination margin is thin, so the
        # distinguisher is weak (bad guesses may also yield a small-support W); the point is
        # only that the adaptation is well-defined and feasible when Eq.(4) holds. On the
        # real parameter sets the margin is ample (see the sweep in briaud_adapted_complexity.py).
        run(seed=2001, m=8, n2=10, k2=4, lblocks=(1, 1), trials_bad=3)
